...
{
	if(theList->head->next == NULL || theList->head->next->next == NULL)
	{
		return;
	}
	Node * pos = theList->head->next;
	while(pos != NULL)
	{
		Node * cur = pos->next;
		Node * smallest = pos;
		while(cur != NULL)
		{
			if((*compare)(pos->data, cur->data) > 0)
			{
				smallest = cur;
			}
			cur = cur->next;
